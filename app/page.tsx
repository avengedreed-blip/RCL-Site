...
import { ProjectMediaImage } from "@/components/ProjectMedia";
      ...
      <section className="hero-section" style={{ alignItems: "start", minHeight: "80vh" }}>
        <Reveal className="hero-copy relative z-10">
          <p className="mb-6 text-sm font-black uppercase text-rcl-red">
            Offline-first software and games
          </p>
          ...
          <p className="mt-5 max-w-[440px] text-lg leading-8 text-rcl-muted">
            Independent software and games built for ownership, privacy, and
            lasting value.
          </p>
            ...
            <div className="relative z-10 grid gap-10 xl:grid-cols-[minmax(0,0.82fr)_minmax(620px,1.18fr)] xl:items-center xl:gap-16">
              <div className="min-w-0">
                ...
                <p className="mt-7 max-w-[520px] text-base leading-8 text-rcl-muted md:text-lg md:leading-9">
                  We build software and games that respect your time, your
                  privacy, and your ownership.
                </p>
              </div>
        ...
        <div className="grid gap-4 lg:grid-cols-2">
          {activeDevelopmentProjects.map((project, index) => (
            <Reveal key={project.slug} delay={index * 0.06}>
              <Link
                href={project.route}
                className="group grid min-h-[560px] overflow-hidden rounded-[6px] border border-white/12 bg-rcl-surface transition duration-300 hover:border-rcl-red/70 hover:bg-rcl-elevated hover:shadow-[0_0_60px_rgba(255,32,32,0.12)]"
              >
                <div className="relative min-h-[280px] overflow-hidden bg-black">
                  ...
                  />
                  <div className="pointer-events-none absolute inset-0 bg-gradient-to-t from-black/82 via-black/18 to-black/14" />
                  <div className="pointer-events-none absolute inset-y-0 left-0 w-1/3 bg-gradient-to-r from-black/42 to-transparent" />
                </div>
                <div className="flex flex-col p-7 md:p-8">
                  <p className="text-xs font-black uppercase text-rcl-red">
                    {getStatusLabel(project.status)} / {project.name}
                  </p>
                  <div className="mt-4 flex items-start justify-between gap-5">
                    ...
                    <ArrowUpRight
                      className="mt-1 h-6 w-6 shrink-0 text-rcl-muted transition group-hover:translate-x-1 group-hover:-translate-y-1 group-hover:text-rcl-red"
                      aria-hidden="true"
                    />
                  </div>
                  <p className="mt-6 max-w-[620px] text-base leading-8 text-rcl-muted">
        ...
        <Reveal>
          <div className="mb-7 grid gap-3 md:grid-cols-[auto_1fr] md:items-end md:gap-5">
            <div>
              ...
              <p className="mt-3 text-sm leading-6 text-rcl-muted">
                Projects planned after current development priorities.
              </p>
            </div>
            <div className="hidden h-px bg-white/25 md:block" />
          </div>
        ...
        <div className="grid gap-4 lg:grid-cols-3">
          {futureProjects.map((project, index) => (
            <Reveal key={project.slug} delay={index * 0.06}>
              <Link
                href={project.route}
                className="group grid min-h-[500px] overflow-hidden rounded-[6px] border border-white/12 bg-rcl-surface transition duration-300 hover:border-rcl-red/70 hover:bg-rcl-elevated"
              >
                <div className="relative min-h-[150px] overflow-hidden bg-black">
                  <ProjectMediaImage
                    visual={project.visual}
                ...
                <div className="flex flex-col p-6">
                  <p className="text-xs font-black uppercase text-rcl-red">
                    {getProjectDateLabel(project)} / {project.name}
                  </p>
                  <div className="mt-4 flex items-start justify-between gap-5">
                  ...
                  <p className="mt-5 text-sm leading-7 text-rcl-muted">
            ...
            <div className="relative z-10 grid gap-8 lg:grid-cols-[1fr_auto] lg:items-center">
              <div>
                ...
                <p className="mt-5 max-w-[540px] text-base leading-8 text-rcl-muted">
                  See what Reed Creative Labs is releasing, building, and
                  planning next.
                </p>
              </div>
